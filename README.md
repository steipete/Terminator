...
```json
{
  "mcpServers": {
    "terminator": {
      "command": "npx",
      "args": ["-y", "@steipete/terminator-mcp@beta"],
      "env": {
...
- **`TERMINATOR_WINDOW_GROUPING`**: Strategy for how new sessions group into windows/tabs.
  ...
  - `"smart"`: (Default) Tries to find an existing window for the `project_path`. If none, tries to find _any_ Terminator-managed window. Otherwise, creates a new window. (See SDD for full logic).
  - Default: `"smart"`
...
- Find the application that _ran_ `terminator` (this might be your IDE, e.g., "Cursor", or "Terminal" itself if you ran a test script from there).
