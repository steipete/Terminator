...
enum AppleTerminalScripts {
    // Helper function to escape strings for AppleScript string literals
    private static func escapeForAppleScript(_ str: String) -> String {
        return str.replacingOccurrences(of: "\\", with: "\\\\")
                  .replacingOccurrences(of: "\"", with: "\\\"")
    }
    
    // Helper function to escape strings for shell command arguments (single-quoted)
    ...
    static func findOrCreateSessionScript(
        ...
        let escapedNewSessionTitle = escapeForAppleScript(newSessionTitle) // AS escape
        ...
        if windowGroupingStrategy == "project", let projPath = projectPathForGrouping, let projHash = projectHashForGrouping, !projPath.isEmpty {
            let escapedProjPath = escapeForAppleScript(projPath)
            let escapedProjHash = escapeForAppleScript(projHash)
            script += """
            set projectHashForTitleSearch to "\(escapedProjHash)"
            ...
            if not foundProjectWindow then
                log "AppleTerminal: No existing window for project '\(escapedProjPath)'. Creating new window."
                set targetWindow to make new window
            else
                log "AppleTerminal: Found existing window for project '\(escapedProjPath)'."
            end if
            """
    ...
    static func executeCommandScript(
        ...
        let quotedLogFilePathForShell = "'" + escapeForShell(outputLogFilePath) + "'"
        // Escape log file path for usage inside an AppleScript string literal that will be part of a JSON string
        let escapedLogPathForAppleScriptJSON = escapeForAppleScript(outputLogFilePath)
        // Escape completion marker for AppleScript string comparison
        let escapedCompletionMarkerForAppleScript = escapeForAppleScript(completionMarker)

        var shellCommandToExecute: String
        if isForeground {
            // For foreground: ( (actual_command) > logfile 2>&1; echo MARKER >> logfile )
            let shellEscapedCommand = escapeForShell(commandToRunRaw)
            let shellEscapedMarker = escapeForShell(completionMarker)
            shellCommandToExecute = "((\(shellEscapedCommand)) > \(quotedLogFilePathForShell) 2>&1; echo '\(shellEscapedMarker)' >> \(quotedLogFilePathForShell))"
        } else {
        ...
        let appleScriptSafeShellCommand = escapeForAppleScript(shellCommandToExecute)
    ...
    static func getPGIDAppleScript(ttyNameOnly: String) -> String {
        ...
        let escapedShellCommand = escapeForAppleScript(shellCommand)
    ...
    static func createTabInWindowScript(appName: String, windowID: String, newSessionTitle: String, shouldActivateTerminal: Bool) -> String {
        let escapedNewSessionTitle = escapeForAppleScript(newSessionTitle)
        return """
        tell application "\(appName)"
            if not running then error "Terminal application \(appName) is not running."
