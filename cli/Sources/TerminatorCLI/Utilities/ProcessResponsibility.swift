    ...
    static func disclaimParentResponsibility() {
        ...
        if #available(macOS 10.14, *) {
            ...
            if result != 0 {
                ...
                Logger.log(level: .warn, "Failed to set responsibility disclaimer flag (error: \(result)). Continuing without disclaimer.")
            }
        } else {
        ...
        if result == 0 {
            ...
            Logger.log(level: .debug, "Parent process (PID: \(getpid())) exiting to hand over to self-responsible child")
            ...
            Logger.log(level: .info, "Continuing without responsibility disclaimer - permission dialogs may not appear correctly")
        }
    }
}