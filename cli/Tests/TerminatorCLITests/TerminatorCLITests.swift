...
final class TerminatorCLITests: XCTestCase {
    ...
    func testInfoCommand_UnknownTerminalApp_Json() throws {
        ...
        do {
            ...
            XCTAssertEqual(decodedOutput.activeConfiguration["TERMINATOR_APP"] ?? "DIFFERENT_VALUE_FOR_NIL_CASE", "UnknownApp123", "TERMINATOR_APP in JSON should match the unknown app")
        } catch {
    ...
    func testExecCommand_WithProjectGrouping_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--project-path", projectPath, "--command", commandToRun])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with project grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for command execution failure with project grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_WithProjectGrouping_FocusNo_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--project-path", projectPath, "--command", commandToRun, "--focus-mode", "no-focus"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with project grouping and no-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_WithProjectGrouping_FocusForce_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--project-path", projectPath, "--command", commandToRun, "--focus-mode", "force-focus"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with project grouping and force-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithSmartGrouping_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with smart grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for command execution failure with smart grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_WithOffGrouping_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with 'off' grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for command execution failure with 'off' grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_WithProjectGrouping_FocusAuto_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--project-path", projectPath, "--command", commandToRun, "--focus-mode", "auto-behavior"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with project grouping and auto-behavior should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithSmartGrouping_FocusForce_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--command", commandToRun, "--focus-mode", "force-focus"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with smart grouping and force-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithSmartGrouping_FocusNo_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--command", commandToRun, "--focus-mode", "no-focus"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with smart grouping and no-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithSmartGrouping_FocusAuto_ActionFails() throws {
        ...
        let result = try runCommand(arguments: ["exec", tagValue, "--command", commandToRun, "--focus-mode", "auto-behavior"])

        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with smart grouping and auto-behavior should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithOffGrouping_FocusNo_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with off grouping and no-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithOffGrouping_FocusForce_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with off grouping and force-focus should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_WithOffGrouping_FocusAuto_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with off grouping and auto-behavior should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    // MARK: - ExecCommand Tests - iTerm Specific
    ...
    func testExecCommand_ITerm_WithProjectGrouping_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, project grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for iTerm, project grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_ITerm_WithSmartGrouping_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, smart grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for iTerm, smart grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_ITerm_WithOffGrouping_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, off grouping should fail when underlying action fails.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message for iTerm, off grouping. Got: \(result.standardError)"
        )
    }
    ...
    func testExecCommand_ITerm_WithProjectGrouping_FocusForce_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, project grouping, force-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithProjectGrouping_FocusNo_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, project grouping, no-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithProjectGrouping_FocusAuto_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, project grouping, auto-behavior should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithSmartGrouping_FocusForce_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, smart grouping, force-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithSmartGrouping_FocusNo_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, smart grouping, no-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithSmartGrouping_FocusAuto_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, smart grouping, auto-behavior should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithOffGrouping_FocusForce_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, off grouping, force-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithOffGrouping_FocusNo_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, off grouping, no-focus should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
    ...
    func testExecCommand_ITerm_WithOffGrouping_FocusAuto_ActionFails() throws {
        ...
        XCTAssertNotEqual(result.exitCode, ExitCode.success, "Exec command with iTerm, off grouping, auto-behavior should fail.")
        XCTAssertTrue(
            result.standardError.contains("Error executing command:") || result.standardError.contains("session not found"),
            "Stderr should contain a relevant error message. Got: \(result.standardError)"
        )
