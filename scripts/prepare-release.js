...
function checkFileSize() {
  ...
  const tsFiles = exec(`find "${projectRoot}/src" -name "*.ts" -type f`)
    .split("\n")
    .filter((f) => f.trim());
  const swiftFiles = exec(`find "${projectRoot}/cli/Sources" -name "*.swift" -type f`)
    .split("\n")
    .filter((f) => f.trim());
...
function checkSwift() {
  ...
  const appleScriptTestPath = join(
    cliPath,
    "Tests",
    "AppleScriptTests",
    "bin",
    "run_apple_terminal_tests.sh",
  );

  if (existsSync(appleScriptTestPath)) {
    if (
      !execWithOutput(
        `cd "${cliPath}/Tests/AppleScriptTests" && bash bin/run_apple_terminal_tests.sh`,
        "AppleScript tests",
      )
    ) {
  ...
  try {
    if (!verifyAppleScriptConsistency()) {
      logWarning(
        "AppleScript consistency check failed - this is expected for parameterized scripts",
      );
...
function checkLoggerConfiguration() {
  ...
  const missingExports = requiredExports.filter(
    (exp) =>
      !loggerContent.includes(`export ${exp}`) &&
      !loggerContent.includes(`export function ${exp}`) &&
      !loggerContent.includes(`export const ${exp}`),
  );
  ...
  const testResult = exec(
    `node -e "import('./dist/logger.js').then(m => console.log('Logger initialized'))"`,
    {
      env: testEnv,
      allowFailure: true,
    },
...
function buildAndVerifyPackage() {
  ...
  const requiredFiles = ["dist/index.js", "bin/terminator", "README.md", "LICENSE"];
