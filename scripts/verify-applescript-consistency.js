...
function extractAppleScriptsFromSwift(filePath) {
  ...
  const functionPattern =
    /static\s+func\s+(\w+Script)\s*\([^)]*\)\s*->\s*String\s*\{([\s\S]*?)\n\s*\}/g;
  let match;
  ...
  while ((match = functionPattern.exec(content)) !== null) {
    ...
    while ((scriptMatch = tripleQuotePattern.exec(functionBody)) !== null) {
      ...
      if (
        scriptContent.match(
          /\b(tell|end tell|to|on|return|set|get|if|then|else|repeat|with|application)\b/,
        )
      ) {
...
function verifyAppleScriptConsistency() {
  ...
  for (const [, sourceScript] of sourceScripts) {
    ...
    if (!foundMatch) {
      ...
      for (const [testName, testScript] of testScripts) {
        if (
          testName.toLowerCase().includes(sourceScript.name.toLowerCase()) ||
          sourceScript.name.toLowerCase().includes(testName.toLowerCase())
        ) {
          if (sourceScript.hash !== testScript.hash) {
            logError(
              `Content mismatch: ${sourceScript.name} (${sourceScript.file}) ≠ ${testScript.file}`,
            );
            ...
            logSuccess(
              `Match found: ${sourceScript.name} (${sourceScript.file}) ↔ ${testScript.file}`,
            );
            unmatched.delete(testName);
            foundMatch = true;
          }
