...
describe('logger', () => {
    ...
    describe('getLogFilePath', () => {
        ...
        it('should fall back to default when custom path is not writable', async () => {
            ...
            const config = getLoggerConfig();
            expect(config.logFile).toContain('terminator.log');
            expect(consoleError).toHaveBeenCalledWith(expect.stringContaining('Cannot write to log file path'));
    ...
    describe('getLogLevel', () => {
        ...
        it.each(['debug', 'info', 'warn', 'error', 'fatal', 'trace'])('should accept valid log level "%s"', async (level) => {
