...
async function main() {
  ...
  if (!fs.existsSync(SWIFT_CLI_PATH)) {
    logger.fatal(
      `Swift CLI binary not found at expected path: ${SWIFT_CLI_PATH}`,
    );
    await flushLogger();
    process.exit(1);
  }
  try {
    ...
    logger.fatal(
      `Swift CLI binary at ${SWIFT_CLI_PATH} is not executable. Please run 'chmod +x ${SWIFT_CLI_PATH}'.`,
    );
    await flushLogger();
    process.exit(1);
  }

  if (!process.env.MCP_PORT && !process.env.MCP_SERVER_VIA_STDIO) {
    // StdioServerTransport doesn't use MCP_PORT
    logger.warn(
      "MCP_PORT or MCP_SERVER_VIA_STDIO environment variable is not set. This plugin is meant to be run by an MCP host over stdio.",
    );
  ...
  server.setRequestHandler(
    CallToolRequestSchema,
    async (request, call): Promise<ServerResult> => {
      ...
      if (toolName !== terminatorTool.name) {
        throw new McpError(
          ...
          `Tool ${toolName} not found. Available: ${terminatorTool.name}`,
        );
      }

      const receivedArgs = request.params.arguments;
      debugLog(
        `[MainServer] CallTool '${toolName}' receivedArgs (flattened):`,
        receivedArgs,
      );
      ...
      if (typeof (receivedArgs as any).action !== "string") {
        debugLog(
          `[MainServer] CallTool '${toolName}' failed validation: typeof action = ${typeof (receivedArgs as any)?.action}`,
        );
        throw new McpError(
          ErrorCode.InvalidParams,
          `Missing or invalid 'action' (string expected) in arguments for tool ${toolName}`,
        );
      }

      // Validate that receivedArgs contains a 'project_path' (now mandatory)
      if (
        typeof (receivedArgs as any).project_path !== "string" ||
        !(receivedArgs as any).project_path.trim()
      ) {
        debugLog(
          `[MainServer] CallTool '${toolName}' failed validation: typeof project_path = ${typeof (receivedArgs as any)?.project_path}, value = ${(receivedArgs as any)?.project_path}`,
        );
        throw new McpError(
          ErrorCode.InvalidParams,
          `Missing or invalid 'project_path' (non-empty string expected) in arguments for tool ${toolName}`,
        );
      }
      ...
      try {
        const result = await terminatorTool.handler(toolParams, {
        ...
        throw new McpError(
          ...
          `Terminator tool execution failed: ${error.message}`,
        );
      }
    },
  );
  ...
  try {
    ...
    logger.info(
      `Terminator MCP server v${SERVER_VERSION} running via stdio, connected to host.`,
    );
  } catch (error) {
...
main().catch(async (err) => {
  logger.fatal(
    { error: err },
    "Terminator MCP plugin failed to run (uncaught error in main)",
  );
  await flushLogger();
  process.exit(1);
});
