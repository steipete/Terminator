...
export const terminatorTool = {
  ...
  inputSchema: {
    ...
    properties: {
      action: {
        type: "string",
        description:
          "Optional. The operation to perform: 'execute', 'read', 'sessions', 'info', 'focus', or 'kill'. Defaults to 'execute'.",
      ...
      tag: {
        type: "string",
        description:
          'Optional. A unique identifier for the session (e.g., "ui-build", "api-server"). If omitted, a tag will be derived from the project_path.',
        optional: true,
      },
      command: {
        type: "string",
        description:
          "Optional, primarily for action: 'execute'. The shell command to execute. If action is 'execute' and command is empty or omitted, the session will be prepared (cleared, focused if applicable), but no new command is run.",
        optional: true,
      },
      background: {
        type: "boolean",
  ...
  async handler(
    params: TerminatorExecuteParams,
    ...
    if (!effectiveProjectPath) {
      return {
        success: false,
        message: `Error: project_path '${params.project_path}' could not be resolved or is invalid.`,
      };
    }
    ...
    if (
      !tag &&
      ["execute", "read", "kill", "focus"].includes(action) &&
      action !== "sessions" &&
      action !== "info"
    ) {
      const errorMsg =
    ...
    const internalWrapperTimeout =
      Math.max(
        DEFAULT_FOREGROUND_COMPLETION_SECONDS * 1000,
        DEFAULT_BACKGROUND_STARTUP_SECONDS * 1000,
      ) + 60000;

    try {
      const result: SwiftCLIResult = await invokeSwiftCLI(
        cliArgs,
        terminatorEnv,
        context,
        internalWrapperTimeout,
      );
      ...
      if (result.cancelled) {
        return {
          success: false,
          message: "Terminator action cancelled by request.",
        };
      }
      ...
      if (result.exitCode === null) {
        ...
        if (
          ...
          errMsg.includes("Segmentation fault") ||
          errMsg.includes("Illegal instruction")
        ) {
          errorDetails.push(
        ...
        return {
          success: false,
          message: `Terminator Error: Swift CLI process terminated unexpectedly\n\n${fullError}`,
        };
      } else if (result.exitCode === 0) {
        const message = formatCliOutputForAI(
          internalAction,
          result,
          commandOpt,
          tag || undefined,
        ...
        return {
          success: false,
          message: `Terminator Error (Swift CLI Code ${result.exitCode}): ${errMsg}`,
        };
      }
    } catch (error: any) {
      ...
      return {
        success: false,
        message: `Terminator plugin internal error: ${error.message}`,
      };
    }
  },
};
