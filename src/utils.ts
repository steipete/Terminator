...
export function resolveEffectiveProjectPath(
  ...
  try {
    ...
    if (!stats.isDirectory()) {
      debugLog(
        `[Utils] effectiveProjectPath error: path '${absolutePath}' exists but is not a directory.`,
      );
      return null;
    }
  } catch (e: any) {
    debugLog(
      `[Utils] effectiveProjectPath error: fs.statSync failed for '${absolutePath}'. Error: ${e.message}`,
    );
    return null;
  }
...
export function resolveDefaultTag(
  currentTag: string | number | undefined,
...
export function formatCliOutputForAI(
  ...
  if (action === "info") {
    try {
      ...
      debugLog(
        `[Utils] CRITICAL ERROR in info parsing: ${e.message}. Stack: ${e.stack}. Raw input was: >>>${stdoutTrimmed}<<<`,
      );
  ...
  if (action === "exec" || action === "execute") {
    ...
    const outputIndicatesTimeout =
      stdoutTrimmed.toLowerCase().includes("execution timed out") ||
      stderrTrimmed.toLowerCase().includes("execution timed out");
