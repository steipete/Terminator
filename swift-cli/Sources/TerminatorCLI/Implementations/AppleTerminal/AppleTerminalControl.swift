...
struct AppleTerminalControl: TerminalControlling {
    ...
    func listSessions(filterByTag: String?) throws -> [TerminalSessionInfo] {
        ...
        case let .success(resultStringOrArray):
            // Logger.log(level: .debug, "AppleScript result for Terminal.app listing: \(resultStringOrArray)") // Can be very verbose
    ...
    func executeCommand(params: ExecuteCommandParams) throws -> ExecuteCommandResult {
        ...
        if isForeground {
            let shellEscapedCommand = trimmedCommandToExecute.replacingOccurrences(of: "'", with: "'\\\\\\\\'\'") // commandToRunRaw -> trimmedCommandToExecute
        ...
        case let .success(submissionResponseString):
            // submissionResponseString is Any, but AppleTerminalScripts.simpleExecuteShellCommandInTabScript returns "OK_COMMAND_SUBMITTED" as String
            guard let responseStr = submissionResponseString as? String, responseStr == "OK_COMMAND_SUBMITTED" else {
                Logger.log(level: .error, "[AppleTerminalControl] Command submission script returned unexpected response or non-string: \\(submissionResponseString). Log: \\(logFilePath.path)")
            ...
            if isForeground {
                ...
                while Date().timeIntervalSince(startTime) < Double(pollTimeoutSeconds) && !markerFoundByPolling {
                    ...
                    // historyAny is Any, AppleTerminalScripts.getTabHistoryScript returns a String
                    if case let .success(historyAny) = historyResult, let historyText = historyAny as? String {
    ...
    func killProcessInSession(params: KillSessionParams) throws -> KillSessionResult {
        ...
        if config.preKillScriptPath == nil && (pgidToKill == nil || !killSuccess) {
            ...
            switch ctrlCResult {
            case let .success(strResult):
                // strResult is Any, AppleTerminalScripts.sendControlCScript returns "OK_CTRL_C_SENT" as String
                if let str = strResult as? String, str == "OK_CTRL_C_SENT" {
    ...
    private static func _clearSessionScreen(appName: String, windowID: String, tabID: String, tag _: String, shouldActivate: Bool) {
    ...
    private func _parsePgidFromResult(resultStringOrArray: Any, tty _: String, tag _: String) -> (pgid: pid_t?, message: String, shouldReturnEarly: Bool) {
        var message = ""
        var pgidToKill: pid_t? = nil
    ...
    private func findOrCreateSessionInternal(
        ...
        if windowGroupingStrategy == .project, let projPath = projectPath {
            let _ = SessionUtilities.generateProjectHash(projectPath: projPath)
            let titleSearchPrefix = "\\(SessionUtilities.PROJECT_HASH_PREFIX)\\(SessionUtilities.generateProjectHash(projectPath: projPath))::"
            for windowInfo in existingWindowsInfo {
                for tabInfo in windowInfo.tabs {
            ...
            if targetWindowID == nil {
                Logger.log(level: .debug, "[AppleTerminalControl] No window found for project '\\(projPath)'. Creating new one.")
            }
        }

        if targetWindowID == nil {
            let createWindowScript = AppleTerminalScripts.createWindowScript(appName: appName, shouldActivate: shouldActivate)
            switch AppleScriptBridge.runAppleScript(script: createWindowScript) {
            case let .success(idString):
                // idString is Any, AppleTerminalScripts.createWindowScript returns an ID as String
                targetWindowID = idString as? String
                if targetWindowID == nil {
                    Logger.log(level: .error, "Could not parse new window ID from AppleScript result (expected String): \\(idString)")
                }
            case let .failure(error):
        ...
        let createTabScript = AppleTerminalScripts.createTabInWindowScript(appName: appName, windowID: finalWindowID, newSessionTitle: newSessionTitle, shouldActivate: shouldActivate, selectTab: selectTabOnCreation)
        ...
        case let .success(tabResultData):
            // If tabResultData is NOT a list (compiler implies it thinks tabResultData is always String here).
            // This means the AppleScript unexpectedly returned a single string instead of a list or an error object.
            if !(tabResultData is [Any]) { // Compiler warns: cast from 'String' to '[Any]' always fails (i.e. tabResultData is String here)
                // If the compiler is right and tabResultData is always a String here, this cast will succeed.
                let errorMessage = "Create tab script returned a plain string instead of a list: \\(tabResultData as! String)"
                throw TerminalControllerError.appleScriptError(message: errorMessage, scriptContent: createTabScript)
            }
            // If the above didn't throw, tabResultData *should* be [Any].
            // However, compiler might still warn on the next line if it's stuck on tabResultData being String.
            guard let resultArray = tabResultData as? [Any], resultArray.count == 4 else {
                throw TerminalControllerError.appleScriptError(message: "Create tab script returned a list but with unexpected item count. Expected 4. Got: \( (tabResultData as? [Any])?.count ?? -1). Data: \(tabResultData)", scriptContent: createTabScript, underlyingError: nil)
            }
