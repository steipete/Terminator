...
struct ITermControl: TerminalControlling {
    ...
    func executeCommand(params: ExecuteCommandParams) throws -> ExecuteCommandResult {
        ...
        Logger.log(level: .debug, "[ITermCtrl] Executing in session ([iTermSessID: \(sessionID), TTY: \(tty ?? "N/A")]): \(appleScriptSafeShellCommand). Log: \(logFilePathURL.path). Marker: \(completionMarker)")
        ...
        case let .success(resultData): // resultData is now a simple status string
            return try _processExecuteCommandWithFileLogging(
                appleScriptStatusString: resultData as? String ?? "ERROR_UNEXPECTED_AS_RESPONSE_TYPE_ITCS", // Changed from as! to as? with fallback
    ...
    private func _processExecuteCommandWithFileLogging(
        appleScriptStatusString: String, // Changed from resultData
        ...
        if !appleScriptStatusString.uppercased().contains("OK") { // e.g. "OK_COMMAND_SUBMITTED" or just "OK"
            Logger.log(level: .error, "[ITermControl] iTerm execute command AppleScript reported error: \\(appleScriptStatusString)", file: #file, function: #function)
            // Attempt to clean up log file if script submission failed.
            try? FileManager.default.removeItem(atPath: logFilePath)
            throw TerminalControllerError.appleScriptError(message: "iTerm execute script error: \\(appleScriptStatusString)", scriptContent: scriptContent)
        }
        ...
        if isForeground {
            ...
            if wasKilledByTimeout {
                Logger.log(level: .warn, "[ITermControl] Command '\\(commandToExecute)' for tag \\(params.tag) timed out after \\(timeoutForThisOperation)s waiting for marker.", file: #file, function: #function)
                ...
                Logger.log(level: .info, "[ITermControl] Foreground iTerm command '\\(commandToExecute)' completed. Log: \\(logFilePath).", file: #file, function: #function)
            }
        } else { // Background
            Logger.log(level: .info, "[ITermControl] Background command '\(commandToExecute)' submitted for tag \(params.tag). Capturing initial output from \(logFilePath) with timeout \(timeoutForThisOperation)s.", file: #file, function: #function)
            ...
            Logger.log(level: .info, "[ITermControl] Background iTerm command submitted. Initial output check complete. Log: \\(logFilePath).")
        }
    ...
    private func findOrCreateSessionForITerm(
        ...
        Logger.log(level: .debug, "[ITermControl] (Refactored) Finding or creating iTerm session. Title: '\(newSessionTitle)', Activate: \(shouldActivate), Profile: \(defaultProfileName)")
        ...
        if config.windowGrouping == .project, projectPath != nil {
            ...
            case let .failure(error):
                Logger.log(level: .warn, "[ITermControl] Failed to list iTerm windows for grouping: \(error.localizedDescription). Proceeding without window list.")
            }
        }
        ...
        if config.windowGrouping == .project, let projPath = projectPath {
            ...
            for winInfo in existingWindows {
                if winInfo.windowName.contains(searchMarker) {
                    ...
                    Logger.log(level: .debug, "[ITermControl] Found existing iTerm window '\(winInfo.windowName)' (ID: \(targetWindowID!)) for project grouping: \(projPath)")
                    break
                }
            }
        }
        ...
        if targetWindowID == nil && config.windowGrouping == .smart {
            Logger.log(level: .debug, "[ITermControl] Window grouping is .smart and no project window found. Trying to use current iTerm window.")
            ...
            if case let .success(winIdStr) = currentWinResult, let winId = winIdStr as? String, !winId.isEmpty, winId != "ERROR" { // Keeping as? String
                targetWindowID = winId
                Logger.log(level: .debug, "[ITermControl] Using current iTerm window ID: \\(winId) for .smart grouping.")
            } else {
                Logger.log(level: .debug, "[ITermControl] Could not get current iTerm window for .smart grouping, or iTerm not active. Will create new window.")
            }
        }
        ...
        if let existingWinID = targetWindowID { // Found a window for project grouping or current (if implemented)
            Logger.log(level: .info, "[ITermControl] Creating new tab in existing iTerm window ID: \(existingWinID)")
            ...
            Logger.log(level: .info, "[ITermControl] Creating new iTerm window.")
            ...
            case let .success(data):
                ...
                if config.windowGrouping == .project, let projPath = projectPath {
                    ...
                    if case let .failure(err) = setNameResult {
                        Logger.log(level: .warn, "[ITermControl] Failed to set new iTerm window name for project grouping: \(err.localizedDescription)")
                    }
                }
            case let .failure(error):
        ...
        if case let .failure(error) = setSessionNameResult {
            ...
            Logger.log(level: .warn, "[ITermControl] Failed to set iTerm session name to '\(newSessionTitle)': \(error.localizedDescription)")
        }
        ...
        if shouldActivate {
            ...
            if case let .failure(error) = focusResult {
                Logger.log(level: .warn, "[ITermControl] Post-creation focus attempt failed for iTerm session \(newSessionData.sessionID): \(error.localizedDescription)")
            }
        }
    ...
    private static func _clearSessionScreen(appName: String, sessionID: String, tag _: String) {
        ...
        if case .failure = clearScriptResult {
