...
export function expectSuccessOrAppleScriptError(result: {
  ...
  // Unexpected error
  throw new Error(
    `Expected success (0) or AppleScript error (3), got exit code ${result.exitCode}: ${result.stderr}`,
  );
}

export function expectFailureWithMessage(
